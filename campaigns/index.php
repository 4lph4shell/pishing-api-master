...
require_once '../config.php'; // Adjust the path if config.php is in the parent directory
...
if ($details === FALSE) {
  $details = json_encode(['org' => 'default_value']);
}
$details = json_decode($details);

// Check if 'org' property exists before accessing it
if (isset($details->org)) {
  $org = $details->org;
} else {
  $org = 'default_value'; // or handle the missing property appropriately
}
  ...
  <body>
    ...
    <CENTER>
      ...
      if (isset($_REQUEST['content'])) {
        ...
        if (isset($_REQUEST['variable1name'])) {
          $variable1name = $_REQUEST['variable1name'];
        } else {
          $variable1name = "";
        }
        if (isset($_REQUEST['variable2name'])) {
          $variable2name = $_REQUEST['variable2name'];
        } else {
          $variable2name = "";
        }
        if (isset($_REQUEST['variable3name'])) {
          $variable3name = $_REQUEST['variable3name'];
        } else {
          $variable3name = "";
        }
        if (isset($_REQUEST['variable4name'])) {
          $variable4name = $_REQUEST['variable4name'];
        } else {
          $variable4name = "";
        }
        if (isset($_REQUEST['variable5name'])) {
          $variable5name = $_REQUEST['variable5name'];
        } else {
          $variable5name = "";
        }
        if (isset($_REQUEST['variable6name'])) {
          $variable6name = $_REQUEST['variable6name'];
        } else {
          $variable6name = "";
        }
        if (isset($_REQUEST['variable7name'])) {
          $variable7name = $_REQUEST['variable7name'];
        } else {
          $variable7name = "";
        }
        if (isset($_REQUEST['variable8name'])) {
          $variable8name = $_REQUEST['variable8name'];
        } else {
          $variable8name = "";
        }
        if (isset($_REQUEST['variable9name'])) {
          $variable9name = $_REQUEST['variable9name'];
        } else {
          $variable9name = "";
        }
        if (isset($_REQUEST['variable10name'])) {
          $variable10name = $_REQUEST['variable10name'];
        } else {
          $variable10name = "";
        }
      ...
      ?>
              ...
              <FORM ACTION="<?php $_SERVER["PHP_SELF"]; ?>" METHOD="POST" enctype="multipart/form-data"><br>
                ...
                <TABLE BORDER="0">
                  <TR>
                    ...
                    <TH><INPUT TYPE="TEXT" Value="<?php echo $campaign; ?>" Name="campaign" ID="campaigntitle"></TH>
                  </TR>
                ...
                if ($campaign != "New") {
                  ...
                  ?>
                    ...
                    <TR>
                      ...
                      <TD><input type="text" placeholder="someone@target.com" id="recipientemail"></TD>
                    </TR>

                    <?php
                    if ($row1['Variable1Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable1Name'] . "\" id=\"variable1name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable2Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable2Name'] . "\" id=\"variable2name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable3Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable3Name'] . "\" id=\"variable3name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable4Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable4Name'] . "\" id=\"variable4name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable5Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable5Name'] . "\" id=\"variable5name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable6Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable6Name'] . "\" id=\"variable6name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable7Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable7Name'] . "\" id=\"variable7name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable8Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable8Name'] . "\" id=\"variable8name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable9Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable9Name'] . "\" id=\"variable9name\" value=\"\"></TD></TR>";
                      $x = $x + 1;
                    }
                    if ($row1['Variable10Name'] != "") {
                      echo "<TR><TD>Variable " . $x . ": </TD><TD><input type=\"text\" placeholder=\"" . $row1['Variable10Name'] . "\" id=\"variable10name\" value=\"\"></TD></TR>";
                    }

                    ?>

                    <TR>
                      <TD COLSPAN="2"><b>Embed Notification for Opened Email? <input type="checkbox" checked id="embednotification"></b></TD>
                    </TR>
        ...
        <h2>
          <font color="#FFFFFF">Select Campaign</FONT>
        </h2>
        ...
        <BR>
        <FONT COLOR="#FFFFFF">
          <H2>OR</H2><br>Use Your Own and Embed a Notification Tag
        </FONT><BR><BR>
...
if ($allowed == "- *Jedi Mind Trick Successful* -") {
  $jedi = 1;
} else {
  $jedi = 0;
}
